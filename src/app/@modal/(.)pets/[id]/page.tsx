...
export default function PetInterceptedPage({ params }: { params: { id: string } }) {
  const petId = params.id;
  const pet = PET_DATA[petId as keyof typeof PET_DATA] || PET_DATA['oliver'];

  return (
    <PetModal>
      <div className="bg-surface-container-lowest w-full max-w-4xl overflow-hidden shadow-2xl relative flex flex-col md:flex-row h-[85vh] md:h-[600px] rounded-[2.5rem]">
        ...
        <div className="w-full md:w-1/2 p-8 md:p-10 flex flex-col overflow-y-auto">
          <div className="flex justify-between items-start mb-6">
            <div>
              <h2 className="font-display text-4xl font-black text-on-surface tracking-tight mb-1">{pet.name}</h2>
